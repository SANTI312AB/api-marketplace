<?php

namespace App\Repository;

use App\Entity\Categorias;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Doctrine\Persistence\ManagerRegistry;

/**
 * @extends ServiceEntityRepository<Categorias>
 *
 * @method Categorias|null find($id, $lockMode = null, $lockVersion = null)
 * @method Categorias|null findOneBy(array $criteria, array $orderBy = null)
 * @method Categorias[]    findAll()
 * @method Categorias[]    findBy(array $criteria, array $orderBy = null, $limit = null, $offset = null)
 */
class CategoriasRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, Categorias::class);
    }

//    /**
//     * @return Categorias[] Returns an array of Categorias objects
//     */
//    public function findByExampleField($value): array
//    {
//        return $this->createQueryBuilder('c')
//            ->andWhere('c.exampleField = :val')
//            ->setParameter('val', $value)
//            ->orderBy('c.id', 'ASC')
//            ->setMaxResults(10)
//            ->getQuery()
//            ->getResult()
//        ;
//    }

//    public function findOneBySomeField($value): ?Categorias
//    {
//        return $this->createQueryBuilder('c')
//            ->andWhere('c.exampleField = :val')
//            ->setParameter('val', $value)
//            ->getQuery()
//            ->getOneOrNullResult()
//        ;
//    }

public function findCategoriesWithProducts()
{
    return $this->createQueryBuilder('c')
        ->leftJoin('c.productos', 'p')
        ->groupBy('c.id')
        ->having('COUNT(p) > 0')
        ->where('c.publicado = :value')
        ->setParameter('value', true)
        ->orderBy('COUNT(p)', 'DESC')  // Ordenar por el número de productos en orden descendente
        ->getQuery()
        ->getResult();
}

  public function categorias_con_productos($producto_tipo =null){


    $query = $this->getEntityManager()->createQueryBuilder();

      $query
      ->select('c.nombre AS nombre','c.slug AS slug', 'COUNT(p.id) AS total_productos')
      ->from('App:categorias', 'c')
      ->Join('c.productos', 'p')
      ->Join('p.productos_tipo', 'pt')
      ->where('p.disponibilidad_producto = :value')
      ->andWhere('p.suspendido = :value2')
      ->andWhere('p.borrador = :value3')
      ->andWhere('p.productos_tipo != :tipo_product')
      ->setParameter('value', true)
      ->setParameter('value2', false)
      ->setParameter('value3', false)
      ->setParameter('tipo_product', 3)
      ->groupBy('c.id')
      ->addGroupBy('c.nombre')
      ->orderBy('COUNT(c.id)', 'DESC')
      ;

      if($producto_tipo !== null) {
        $query->andWhere('pt.tipo = :filtro_tipo')
            ->setParameter('filtro_tipo', $producto_tipo);
      }else{
        $query->andWhere('pt.tipo = :filtro_tipo')
        ->setParameter('filtro_tipo', 'FISICO');
      }


      return $query->getQuery()->getResult();

  }

}
